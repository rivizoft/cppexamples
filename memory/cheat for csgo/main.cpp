...
int main() 
{
	...
	clientBase = memory.getModule("client_panorama.dll"); // ищем клиент кс го

	std::cout << "clientBase " << clientBase.dwAddress << std::endl; //выводим адрес
	
	while (true)
	{
		// облась памяти нашего игрока - Начальный адрес + оффсет 0xCFAA3C
		DWORD player = memory.read<DWORD>(clientBase.dwAddress + oPlayer);
		printf("player: 0x%X\n", player);
		// Полученный адрес игрока + оффсет здоровья 0x100
		DWORD health = memory.read<DWORD>(player + oHealth);
		printf("health: %d\n", health);
		// Адрес игрока + оффсет номер команды 0xF4
